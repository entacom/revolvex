...
$pdf = new MYPDF($order_number, $order_delivery_date, $order_user);
...
while ($part_number = $stmt->fetchColumn()) {
    // Add a new page for each part_number
    $pdf->AddPage('P');
$pdf->ln(2);
...
';
    ...
    $query2 = "SELECT * FROM tblOrderItems WHERE order_id = :order_id AND part_number = :part_number";
    $stmt2 = $conn->prepare($query2);
    $stmt2->bindValue(':order_id', $_GET['order_id'], PDO::PARAM_INT);
    $stmt2->bindValue(':part_number', $part_number, PDO::PARAM_STR);
    $stmt2->execute();
    ...
    if ($stmt2->rowCount() > 0) {
        while ($row = $stmt2->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($row['has_items']) {
                $query3 = "SELECT * FROM tblOrderSubItems WHERE order_group_id = :order_group_id  ORDER BY pack_id ASC";
                $stmt3 = $conn->prepare($query3);
                $stmt3->bindValue(':order_group_id', $row['id'], PDO::PARAM_INT);
                $stmt3->execute();
    ...
    // Add signature lines after the table on each page
    $signature_lines = '
...
$pdf->Output($_GET['order_id'] . '_production_cards.pdf', 'I');
