...
/* -------------------- PACK RANGE -------------------- */
$max_pack_no = (int)getMaxPackOrder($order_id, $_SESSION['session_company_id']);
if ($max_pack_no <= 0) {
    http_response_code(404);
    exit('No packs found for this order');
}
...
for ($pack_no = 1; $pack_no <= $max_pack_no; $pack_no++) {
    ...
    /* only keep centre divider - header removed */
    ...
    for ($rr = 0; $rr < $rows; $rr++) {
        for ($c = 0; $c < 2; $c++) {
            if ($idx < count($tokens)) {
                $pdf->Text($colX[$c], $startY + ($rr * $rowH), $tokens[$idx]);
            }
            $idx++;
        }
    }
...
if ($pagesMade === 0) {
    http_response_code(404);
    exit('No pack items found to print.');
}

if (ob_get_length()) {
    ob_end_clean();
}
$pdf->Output('label_order' . $order_id . '-ALL.pdf', 'I');
