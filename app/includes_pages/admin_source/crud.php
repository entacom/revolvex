...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $data_raw = json_decode(file_get_contents("php://input"), true);
        ...
        if (isset($data_raw['action']) && $data_raw['action'] == 'add_source' ) {
            if (empty($data_raw['unit_description'])) {
                ...
                $query = "INSERT INTO tblClientSource (company_id, description) VALUES (:company_id, :source_description)";
                $bindings = array(
    ...
    if (isset($data_raw['action']) && $data_raw['action'] == 'edit_source') {	
        $data = sanInputs($data_raw);
        $database = new Database();
        $conn = $database->connect();
        $query = "UPDATE tblClientSource SET description = :unit_description WHERE id = :id AND company_id = :company_id";
                    $bindings = array(
                        ':id' => $data['id'],
    ...
    if (isset($data_raw['action']) && $data_raw['action'] == 'read_unit') {
        ...
        $conn = $database->connect();
        $return_arr = array();
        $query = "SELECT * FROM tblClientSource WHERE id = :id";
...
if (isset($data_raw['action']) && $data_raw['action'] == 'toggle_active') {
    try {
        ...
        $stmt = $conn->prepare("SELECT active FROM tblClientSource WHERE id = :id AND company_id = :company_id");
        ...
        $stmt = $conn->prepare("UPDATE tblClientSource SET active = :new_status WHERE id = :id AND company_id = :company_id");
