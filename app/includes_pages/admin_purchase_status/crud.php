...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $data_raw = json_decode(file_get_contents("php://input"), true);
        ...
        if (isset($data_raw['action']) && $data_raw['action'] == 'add_status' ) {
            if (empty($data_raw['description'])) {
                ...
                $query = "INSERT INTO tblPurchaseStatus (company_id, description) VALUES (:company_id, :description)";
                $bindings = array(
    ...
    if (isset($data_raw['action']) && $data_raw['action'] == 'edit_status') {	
        ...
        $database = new Database();
        $conn = $database->connect();
        $query = "UPDATE tblPurchaseStatus SET description = :description WHERE id = :id AND company_id = :company_id";
                    $bindings = array(
                        ':id' => $data['id'],
                        ':description' => $data['description'],
                        ':company_id' => $_SESSION['session_company_id']
    ...
    if (isset($data_raw['action']) && $data_raw['action'] == 'read_status') {
        ...
        $conn = $database->connect();
        $return_arr = array();
        $query = "SELECT * FROM tblPurchaseStatus WHERE id = :id";
...
if (isset($data_raw['action']) && $data_raw['action'] == 'toggle_active') {
    try {
        ...
        $stmt = $conn->prepare("SELECT active FROM tblPurchaseStatus WHERE id = :id AND company_id = :company_id");
        ...
        $stmt = $conn->prepare("UPDATE tblPurchaseStatus SET active = :new_status WHERE id = :id AND company_id = :company_id");
