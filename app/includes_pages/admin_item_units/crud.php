...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $data_raw = json_decode(file_get_contents("php://input"), true);
        ...
        if (isset($data_raw['action']) && $data_raw['action'] == 'add_unit' ) {
            if (empty($data_raw['unit_description'])) {
                ...
                $query = "INSERT INTO tblItemUnits (company_id, description, divisible) VALUES (:company_id, :unit_description, :divisible)";
                $bindings = array(
    ...
    if (isset($data_raw['action']) && $data_raw['action'] == 'edit_unit') {	
        $data = sanInputs($data_raw);
        $database = new Database();
        $conn = $database->connect();
        $query = "UPDATE tblItemUnits SET description = :unit_description,divisible = :divisible WHERE id = :id AND company_id = :company_id";
                    $bindings = array(
                        ':id' => $data['id'],
                        ':unit_description' => $data['unit_description'],
                        ':divisible' => $data['divisible'],
    ...
    if (isset($data_raw['action']) && $data_raw['action'] == 'read_unit') {
        $data = sanInputs($data_raw);
        $database = new Database();
        $conn = $database->connect();
        $return_arr = array();
        $query = "SELECT * FROM tblItemUnits WHERE id = :id";
        $result = $conn->prepare($query);
        $result->bindParam(':id', $data['unit_id']); 
        $result->execute();
...
if (isset($data_raw['action']) && $data_raw['action'] == 'toggle_active') {
    try {
        ...
        $stmt = $conn->prepare("SELECT active FROM tblItemUnits WHERE id = :id AND company_id = :company_id");
        ...
        $stmt = $conn->prepare("UPDATE tblItemUnits SET active = :new_status WHERE id = :id AND company_id = :company_id");
